...
class AppMain:
    def __init__(self):
        ...
        self.world = particle1.World(width, height, dt=1.0, gy=0.5)
        self.particle_list = []
    ...
    def add_particle(self, pos, button):
        ...
        if button == 1:
            p = particle1.Particle(pos, (vx, vy), self.world, color="green")
        elif button == 3:
            p = particle1.Particle(pos, (vx, vy), self.world, color="blue", \
                                    postmove_strategy=particle1.BounceOnBoundaryStrategy())
        else:
            return
