...
class AppMain:
    def __init__(self):
        ...
        self.world = particle2.World(width, height, dt=1.0, gy=0.5)
        self.particle_list = []
    ...
    def add_particle(self, pos, button):
        ...
        if button == 1:
            p = particle2.Particle(pos, (vx, vy), self.world, color="green")
        elif button == 3:
            p = particle2.Particle(pos, (vx, vy), self.world, color="blue", \
                                    postmove_strategy=particle2.BounceOnBoundaryStrategy())
        else:
            return
